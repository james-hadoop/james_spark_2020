...
package org.apache.spark.ml.classification.james
...
import org.apache.spark.ml.classification.{ProbabilisticClassificationModel, ProbabilisticClassifier, ProbabilisticClassifierParams}
 ...
 */
private[classification] trait JamesLogisticRegressionParams extends ProbabilisticClassifierParams
 ...
 */
@Since("1.2.0")
class JamesLogisticRegression @Since("1.2.0") (
                                           @Since("1.4.0") override val uid: String)
  extends ProbabilisticClassifier[Vector, JamesLogisticRegression, JamesLogisticRegressionModel]
    with JamesLogisticRegressionParams with DefaultParamsWritable with Logging {
  ...
  private var optInitialModel: Option[JamesLogisticRegressionModel] = None

  private[spark] def setInitialModel(model: JamesLogisticRegressionModel): this.type = {
    this.optInitialModel = Some(model)
    this
  }

  override protected[spark] def train(dataset: Dataset[_]): JamesLogisticRegressionModel = {
  ...
  protected[spark] def train(
                              dataset: Dataset[_],
                              handlePersistence: Boolean): JamesLogisticRegressionModel = instrumented { instr =>
    ...
    val (coefficientMatrix, interceptVector, objectiveHistory) = {
      ...
      if ($(fitIntercept) && isConstantLabel && !usingBoundConstrainedOptimization) {
        ...
        if (!$(fitIntercept) && (0 until numFeatures).exists { i =>
          ...
          instr.logWarning("Fitting JamesLogisticRegressionModel without intercept on dataset with " +
    ...
    val model = copyValues(new JamesLogisticRegressionModel(uid, coefficientMatrix, interceptVector,
      numClasses, isMultinomial))

    val (summaryModel, probabilityColName, predictionColName) = model.findSummaryModel()
    val logRegSummary = if (numClasses <= 2) {
      new BinaryJamesLogisticRegressionTrainingSummaryImpl(
      ...
      new JamesLogisticRegressionTrainingSummaryImpl(
        summaryModel.transform(dataset),
        probabilityColName,
        predictionColName,
        $(labelCol),
  ...
  @Since("1.4.0")
  override def copy(extra: ParamMap): JamesLogisticRegression = defaultCopy(extra)
}

@Since("1.6.0")
object JamesLogisticRegression extends DefaultParamsReadable[JamesLogisticRegression] {

  @Since("1.6.0")
  override def load(path: String): JamesLogisticRegression = super.load(path)
...
/**
 * Model produced by [[JamesLogisticRegression]].
 */
@Since("1.4.0")
class JamesLogisticRegressionModel private[spark] (
                                               @Since("1.4.0") override val uid: String,
  ...
  extends ProbabilisticClassificationModel[Vector, JamesLogisticRegressionModel]
    with JamesLogisticRegressionParams with MLWritable {
  ...
  private lazy val _coefficients: Vector = {
    require(coefficientMatrix.isTransposed,
      "JamesLogisticRegressionModel coefficients should be row major for binomial model.")
  ...
  private var trainingSummary: Option[JamesLogisticRegressionTrainingSummary] = None
   ...
   */
  @Since("1.5.0")
  def summary: JamesLogisticRegressionTrainingSummary = trainingSummary.getOrElse {
    throw new SparkException("No training summary available for this JamesLogisticRegressionModel")
  }
   ...
   */
  @Since("2.3.0")
  def binarySummary: BinaryJamesLogisticRegressionTrainingSummary = summary match {
    case b: BinaryJamesLogisticRegressionTrainingSummary => b
    case _ =>
  ...
  (JamesLogisticRegressionModel, String, String) = {
  ...
  def setSummary(summary: Option[JamesLogisticRegressionTrainingSummary]): this.type = {
   ...
   */
  @Since("2.0.0")
  def evaluate(dataset: Dataset[_]): JamesLogisticRegressionSummary = {
    // Handle possible missing or invalid prediction columns
    val (summaryModel, probabilityColName, predictionColName) = findSummaryModel()
    if (numClasses > 2) {
      new JamesLogisticRegressionSummaryImpl(summaryModel.transform(dataset),
        probabilityColName, predictionColName, $(labelCol), $(featuresCol))
    } else {
      new BinaryJamesLogisticRegressionSummaryImpl(summaryModel.transform(dataset),
        probabilityColName, predictionColName, $(labelCol), $(featuresCol))
    }
  }
  ...
  override protected def raw2probabilityInPlace(rawPrediction: Vector): Vector = {
    rawPrediction match {
      ...
      case sv: SparseVector =>
        throw new RuntimeException("Unexpected error in JamesLogisticRegressionModel:" +
  ...
  @Since("1.4.0")
  override def copy(extra: ParamMap): JamesLogisticRegressionModel = {
    val newModel = copyValues(new JamesLogisticRegressionModel(uid, coefficientMatrix, interceptVector,
   ...
   *
   * For [[JamesLogisticRegressionModel]], this does NOT currently save the training [[summary]].
   * An option to save [[summary]] may be added in the future.
   *
   * This also does not save the [[parent]] currently.
   */
  @Since("1.6.0")
  override def write: MLWriter = new JamesLogisticRegressionModel.JamesLogisticRegressionModelWriter(this)

  override def toString: String = {
    s"JamesLogisticRegressionModel: " +
...
@Since("1.6.0")
object JamesLogisticRegressionModel extends MLReadable[JamesLogisticRegressionModel] {

  @Since("1.6.0")
  override def read: MLReader[JamesLogisticRegressionModel] = new JamesLogisticRegressionModelReader

  @Since("1.6.0")
  override def load(path: String): JamesLogisticRegressionModel = super.load(path)

  /** [[MLWriter]] instance for [[JamesLogisticRegressionModel]] */
  private[JamesLogisticRegressionModel]
  class JamesLogisticRegressionModelWriter(instance: JamesLogisticRegressionModel)
  ...
  private class JamesLogisticRegressionModelReader extends MLReader[JamesLogisticRegressionModel] {

    /** Checked against metadata when loading model */
    private val className = classOf[JamesLogisticRegressionModel].getName

    override def load(path: String): JamesLogisticRegressionModel = {
      ...
      val model = if (major.toInt < 2 || (major.toInt == 2 && minor.toInt == 0)) {
        ...
        new JamesLogisticRegressionModel(metadata.uid, coefficientMatrix,
        ...
        coefficientMatrix: Matrix, isMultinomial: Boolean) = data
          .select("numClasses", "numFeatures", "interceptVector", "coefficientMatrix",
            "isMultinomial").head()
        new JamesLogisticRegressionModel(metadata.uid, coefficientMatrix, interceptVector,
          numClasses, isMultinomial)
      }
 ...
 */
@Experimental
sealed trait JamesLogisticRegressionSummary extends Serializable {
   ...
   */
  @Since("2.3.0")
  def asBinary: BinaryJamesLogisticRegressionSummary = this match {
    case b: BinaryJamesLogisticRegressionSummary => b
    case _ =>
 ...
 */
@Experimental
sealed trait JamesLogisticRegressionTrainingSummary extends JamesLogisticRegressionSummary {
 ...
 */
@Experimental
sealed trait BinaryJamesLogisticRegressionSummary extends JamesLogisticRegressionSummary {
   ...
   *
   * @note This ignores instance weights (setting all to 1.0) from `JamesLogisticRegression.weightCol`.
 ...
 */
@Experimental
sealed trait BinaryJamesLogisticRegressionTrainingSummary extends BinaryJamesLogisticRegressionSummary
  with JamesLogisticRegressionTrainingSummary
 ...
 */
private class JamesLogisticRegressionTrainingSummaryImpl(
  ...
  extends JamesLogisticRegressionSummaryImpl(
 ...
 * @param featuresCol field in "predictions" which gives the features of each instance as a vector.
 */
private class JamesLogisticRegressionSummaryImpl(
                                             @transient override val predictions: DataFrame,
                                             override val probabilityCol: String,
                                             override val predictionCol: String,
                                             override val labelCol: String,
                                             override val featuresCol: String)
  extends JamesLogisticRegressionSummary
 ...
 * @param objectiveHistory objective function (scaled loss + regularization) at each iteration.
 */
private class BinaryJamesLogisticRegressionTrainingSummaryImpl(
                                                           predictions: DataFrame,
                                                           probabilityCol: String,
                                                           predictionCol: String,
                                                           labelCol: String,
                                                           featuresCol: String,
                                                           override val objectiveHistory: Array[Double])
  extends BinaryJamesLogisticRegressionSummaryImpl(
    predictions, probabilityCol, predictionCol, labelCol, featuresCol)
    with BinaryJamesLogisticRegressionTrainingSummary
 ...
 * @param featuresCol field in "predictions" which gives the features of each instance as a vector.
 */
private class BinaryJamesLogisticRegressionSummaryImpl(
                                                   predictions: DataFrame,
                                                   probabilityCol: String,
                                                   predictionCol: String,
                                                   labelCol: String,
  ...
  extends JamesLogisticRegressionSummaryImpl(
    predictions, probabilityCol, predictionCol, labelCol, featuresCol)
    with BinaryJamesLogisticRegressionSummary
